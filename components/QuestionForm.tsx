...
const QuestionForm = ({ question }: { question?: any }) => {
  ...
  const submit = async (e: React.FormEvent<HTMLFormElement>) => {
    ...
    if (!formData.content.trim() || formData.content.length < 20) {
        setError("Please provide detailed content (min 20 characters).");
        return;
    }
    ...
    try {
      ...
      // Log the full error for debugging but show a readable message
      console.error("Appwrite Error:", err);
      setError(err.message || "An unexpected error occurred while saving your question.");
    } finally {
      setLoading(false);
    }
  };
    ...
    <form className="space-y-6" onSubmit={submit}>
      ...
      <LabelInputContainer>
        ...
        <Input 
            ...
            placeholder="e.g. How to use Appwrite storage with Next.js Server Actions?" 
        />
      </LabelInputContainer>
